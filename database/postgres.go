...
func (repo *PostgresRepository) DeletePost(ctx context.Context, post string, user string) error {
	_, err := repo.db.ExecContext(ctx, "DELETE FROM posts WHERE id = $1 and user_id = $2", post, user)
	return err
}

func (repo *PostgresRepository) ListPosts(ctx context.Context, page uint64) (*[]models.Post, error) {
	rows, err := repo.db.QueryContext(ctx, "SELECT id, post_content, user_id, created_at FROM posts LIMIT $1 OFFSET $2", OFFSET, page*OFFSET)
